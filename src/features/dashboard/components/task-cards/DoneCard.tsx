...
const DONE_CARD_BASE =
  "rounded-2xl border border-l-4 border-l-[#1D9E75] p-4 transition-all duration-700";

const DONE_CARD_TONE = {
  default: "border-border bg-muted opacity-70",
  highlighted:
...
export function DoneCard({
  ...
  const className = useMemo(
    () =>
      cn(
        DONE_CARD_BASE,
        highlighted ? DONE_CARD_TONE.highlighted : DONE_CARD_TONE.default,
      ),
    [highlighted],
  );
  ...
  const highlightProps = useMemo(
    () =>
      installmentId ? { [QUEUE_HIGHLIGHT_ATTR]: installmentId } : undefined,
    [installmentId],
  );
