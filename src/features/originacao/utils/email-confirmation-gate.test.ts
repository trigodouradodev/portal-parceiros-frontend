...
describe("getBlockedEmailField", () => {
  ...
  it("blocks on step 2 (Avalista) when the guarantor e-mail is blocked", () => {
    expect(getBlockedEmailField(2, UNCHECKED, BLOCKED)).toBe("guarantor.email");
  });
  ...
  it.each([UNCHECKED, CHECKING, OK, UNAVAILABLE])(
    "never blocks step 2 for status %s — only blocked does",
    (status) => {
      expect(getBlockedEmailField(2, UNCHECKED, status)).toBeNull();
    },
  );

  it("never blocks a step other than 0 or 2, even if a status is blocked", () => {
  ...
  it("does not confuse the registration e-mail status with the guarantor's", () => {
    ...
    expect(getBlockedEmailField(2, BLOCKED, OK)).toBeNull();
